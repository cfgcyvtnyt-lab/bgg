...
BGG 공식 XML API2(`/xmlapi2/collection`)를 사용합니다. 예전에 쓰던 `geekcollection.php?action=exportcsv`
비공식 내보내기 경로는 BGG의 봇 차단이 강화되면서 아이디만으로는 자주 실패하게 되어 정식 API로 교체했습니다.

## 처음 설정할 것 (한 번만)

1. **새 GitHub 저장소 만들기** (Public이어야 GitHub Pages 무료로 씀). 이 폴더 안의 파일들을
   (`.github/workflows/update-bgg-collection.yml`, `scripts/xml_to_json.py`) 그대로 업로드/커밋.

2. **본인 BGG 아이디 등록**
   저장소 `Settings` → `Secrets and variables` → `Actions` → `Variables` 탭 →
   `New repository variable` → 이름 `BGG_USERNAME`, 값에 본인 BGG 아이디 입력.

3. **BGG API 키 등록 (권장)**
   저장소 `Settings` → `Secrets and variables` → `Actions` → `Secrets` 탭 →
   `New repository secret` → 이름 `BGG_API_KEY`, 값에 발급받은 API 키 입력.
   (공개 컬렉션이면 키 없이도 동작할 수 있지만, BGG가 인증 헤더를 요구하는 쪽으로 바뀌고 있어 넣어두는 걸 권장.)

4. **GitHub Pages 켜기**
   저장소 `Settings` → `Pages` → `Source`를 `Deploy from a branch`로,
   브랜치는 `main`, 폴더는 `/docs`로 지정 → 저장.

5. **한 번 수동 실행해서 첫 데이터 만들기**
   저장소 `Actions` 탭 → `Update BGG collection` 워크플로 선택 → `Run workflow` 버튼 클릭.
   1~2분 후 `docs/collection.json`이 생기고 커밋됩니다.

6. 완료되면 아래 주소에서 JSON이 보여야 합니다 (몇 분 정도 걸릴 수 있음):
